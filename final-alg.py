...
def counting(a):
    if not a or isinstance(a[0], (str, float)):
        return a
    m=max(a)
    c=[0]*(m+1)
    for x in a:
...
def measure(f,data):
    # Safety check for non-integer datasets on integer-only algorithms
    if f.__name__ in ["counting", "radix"] and len(data) > 0:
        if isinstance(data[0], (str, float)):
            return None
    s=time.perf_counter()
    f(data)
    e=time.perf_counter()
...
def task(args):
    ...
    res = []
    if t1 is not None:
        res.append((alg_name,"array",dtype,size,t1))
    if t2 is not None:
        res.append((alg_name,"linked",dtype,size,t2))
